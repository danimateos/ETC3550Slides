...
* Fit a time series linear model like the one we just used to the stock market prices dataset (the `Close` variable in `gafa_stock`). Remember that these data don't have a regular index. 

* Using that model, forecast for the next 120 days and visualize both the forecast and past.

```{r eval=FALSE, include=FALSE}
stock_reindexed = gafa_stock %>% group_by(Symbol) %>% mutate(row = row_number()) %>% update_tsibble(index = row, regular = TRUE) 

stock_reindexed %>% model(tslm = TSLM(Close ~ trend())) %>% forecast(h=120) %>% autoplot(stock_reindexed)
```
...
* Compute naïve, seasonal naïve, mean and drift-method forecasts for quarterly Australian Cement production. Use only data before 2005 for the training.

* Examine the residuals for each of these methods. What do you conclude?

* Use those models to forecast the last 5 years of data and plot the forecasts together with the real data. How does what you see connect with the conclusions you extracted from the residuals plot?

```{r eval=FALSE, include=FALSE}
aus_production %>% filter(year(Quarter) < 2005) %>% model(mean = MEAN(Cement), naive = NAIVE(Cement), snaive = SNAIVE(Cement), drift = RW(Cement ~ drift())) -> fits

fits %>% forecast(h="5 years") %>% autoplot(aus_production)
```
...
* Fit a decomposition model to the Cement data from `aus_production`, again only training with the data prior to 2005.

* Plot the forecasts for the following five years from this model and the best performing model from the earlier exercise

```{r}
aus_production %>% 
  filter(year(Quarter) < 2005) %>% 
  model(drift = RW(Cement ~ drift()),
        decomp = decomposition_model(STL(Cement),
